...
#----------------------- 2） visualization-----------------------------------
# ----- Plot demo A -----: bar plot (percent mode) + chance + sig 
...
self.fig.savefig("figure1.png",dpi=300,transparent=True,bbox_inches="tight")



# ----- Plot demo B -----: paired scatter + KDE
data1 = np.random.normal(loc=0.65, scale=0.2, size=50)  # Mean = 0.65, std = 0.2, 50 samples
data2 = np.random.normal(loc=0.8, scale=0.5, size=50)  # Mean = 0.8, std = 0.5, 50 samples
data = [data1, data2]

self=sp.paired_scatter_kde_plot(data,
                            palette={'A1': '#4a90e2', 'A2': '#e24a33',},
                            xlabel='',
                            ylabel='Values',
                            xticklabels=['A1','A2'],
                            figsize=(4,6),fontsize_xlabel=30, fontsize_ylabel=30, fontsize_xtick=25,y_major_locator=0.6,percent_mode=0,
                            xlim=(-0.5,1.5),
                                  )
self.fig.savefig("figure2.png",dpi=300,transparent=True,bbox_inches="tight")


# ----- Plot demo C -----: Two-factor demo with hue 
data_A1B1 = np.random.normal(loc=0.65, scale=0.1, size=50)
data_A2B1 = np.random.normal(loc=0.9, scale=0.1, size=50)
data_A1B2 = np.random.normal(loc=0.35, scale=0.1, size=50)
data_A2B2 = np.random.normal(loc=0.85, scale=0.1, size=50)
data=sp.data_to_long([[data_A1B1,data_A2B1],[data_A1B2,data_A2B2]],factorA_name='condition A',factorB_name='condition B',factorA_levels=['A1','A2'],factorB_levels=['B1','B2'])

self=sp.two_factor_with_hue(data,x='condition A',y='Value',hue='condition B',
                    xticklabels=['A1','A2'],
                    xlabel='Condition',fontsize_xlabel=23,
                    fontsize_xtick=20,ylabel='Value',y_major_locator=0.5,
                   )
self.fig.savefig("figure3.png",dpi=300,transparent=True,bbox_inches="tight")
